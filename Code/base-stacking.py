...
def base_stack(base, coords, cent_coor1, cent_coor2):
    ...
    if base == 'A' or base == 'G':
        if 'N1' in coords.keys() and 'C8' in coords.keys() and 'N3' in coords.keys():
            ...
            point_coord = np.array([cent_coor2[0] - A * t, cent_coor2[1] - B * t, cent_coor2[2] - C * t])  # 投影点坐标
            bases_Z = np.linalg.norm(cent_coor2 - point_coord)  # 垂直距离，是否共面
            bases_D = np.linalg.norm(cent_coor1 - point_coord)  # 碱基之间的距离,投影与质心的距离
            L = np.linalg.norm(cent_coor1 - cent_coor2)  # 两质心间的距离
            V1 = np.array([N1[0] - cent_coor1[0], N1[1] - cent_coor1[1], N1[2] - cent_coor1[2]])  # x轴方向， 质心到N1
            V2 = np.array(
            ...
            cross_product = np.cross(V1, V2)  # 叉乘
        ...
        if 'N1' in coords.keys() and 'N3' in coords.keys() and 'C4' in coords.keys():
            ...
            point_coord = np.array([cent_coor2[0] - A * t, cent_coor2[1] - B * t, cent_coor2[2] - C * t])  # 投影点坐标
            bases_Z = np.linalg.norm(cent_coor2 - point_coord)  # 垂直距离，是否共面
            bases_D = np.linalg.norm(cent_coor1 - point_coord)  # 碱基之间的距离
            L = np.linalg.norm(cent_coor1 - cent_coor2)   # 两质心间的距离
            V1 = np.array([N3[0] - cent_coor1[0], N3[1] - cent_coor1[1], N3[2] - cent_coor1[2]])  # x轴方向， 质心到N1
            V2 = np.array(
                [point_coord[0] - cent_coor1[0], point_coord[1] - cent_coor1[1], point_coord[2] - cent_coor1[2]])
            V1_mo = np.sqrt(V1.dot(V1))
            V2_mo = np.sqrt(V2.dot(V2))
